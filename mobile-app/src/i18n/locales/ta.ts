/**
 * Tamil Language Translations (தமிழ்)
 * 
 * Tamil language file for Sri Lankan Tamil users.
 */

export default {
  // Common
  common: {
    save: 'சேமி',
    cancel: 'ரத்து செய்',
    edit: 'திருத்து',
    delete: 'நீக்கு',
    confirm: 'உறுதிப்படுத்து',
    loading: 'ஏற்றுகிறது...',
    error: 'பிழை',
    success: 'வெற்றி',
    retry: 'மீண்டும் முயற்சி',
    viewAll: 'அனைத்தையும் பார்',
    call: 'அழைப்பு',
    back: 'பின்',
    next: 'அடுத்து',
    done: 'முடிந்தது',
    add: 'சேர்',
    update: 'புதுப்பி',
    search: 'தேடு',
    noData: 'தரவு இல்லை',
    syncPending: 'ஒத்திசைவு நிலுவையில்',
    synced: 'ஒத்திசைக்கப்பட்டது',
    offline: 'ஆஃப்லைன்',
    online: 'ஆன்லைன்',
  },

  // Navigation
  navigation: {
    home: 'முகப்பு',
    profile: 'சுயவிவரம்',
    vaccines: 'தடுப்பூசிகள்',
    growth: 'வளர்ச்சி',
    feeding: 'உணவு',
    schedule: 'அட்டவணை',    profileRequired: 'சுயவிவரம் தேவை',
    profileRequiredMessage: 'இந்த அம்சத்தைப் பயன்படுத்த கர்ப்ப சுயவிவரம் உருவாக்கவும் அல்லது குழந்தை சுயவிவரத்தை சேர்க்கவும்.',  },

  // Home Screen
  home: {
    title: 'குழந்தை சுகாதாரம்',
    subtitle: 'வளர்ச்சி பதிவு',
    welcomeBanner: 'உங்கள் குடும்பத்திற்கு வரவேற்கிறோம்',
    welcomeSubtitle: 'குழந்தை சுகாதார வளர்ச்சி பதிவு',
    nextAppointment: 'அடுத்த சந்திப்பு',
    growthPercentile: 'வளர்ச்சி சதவீதம்',
    immunizations: 'நோய்த்தடுப்பு',
    overdueVaccines: 'தாமதமான தடுப்பூசிகள்',
    immunizationProgress: 'நோய்த்தடுப்பு முன்னேற்றம்',
    overallProgress: 'ஒட்டுமொத்த முன்னேற்றம்',
    vaccinesCompleted: '{{total}} இல் {{completed}} தடுப்பூசிகள் நிறைவு',
    nextVaccine: 'அடுத்தது: {{vaccine}} - {{age}}',
    addRecord: 'பதிவு சேர்',
    selectRecordType: 'நீங்கள் சேர்க்க விரும்பும் பதிவு வகையைத் தேர்ந்தெடுக்கவும்',
    addGrowthRecord: 'வளர்ச்சி அளவீடு',
    addVaccineRecord: 'தடுப்பூசி பதிவு',
    addAppointment: 'சந்திப்பு திட்டமிடுக',
    recordTitle: 'தலைப்பு',
    recordTitlePlaceholder: 'தலைப்பை உள்ளிடவும்',
    recordDescription: 'விளக்கம்',
    recordDescriptionPlaceholder: 'விளக்கத்தை உள்ளிடவும் (விரும்பமானது)',
    titleRequired: 'தயவுசெய்து தலைப்பை உள்ளிடவும்',
    deleteRecord: 'பதிவை நீக்கு',
    deleteRecordConfirm: 'இந்த பதிவை நீக்க விரும்புகிறீர்களா?',
    aiChat: 'AI அரட்டை',
    emergencyContacts: 'அவசர தொடர்புகள்',
    recentActivities: 'சமீபத்திய நடவடிக்கைகள்',
    dailyHealthTip: 'தினசரி சுகாதார குறிப்பு',
    chdrNumber: 'CHDR எண்:',
    monthsOld: '{{months}} மாதங்கள் {{weeks}} வாரங்கள்',
    weight: 'எடை:',
    height: 'உயரம்:',
    noChildProfile: 'குழந்தை சுயவிவரம் கிடைக்கவில்லை',
    failedToSave: 'செயல்பாட்டைச் சேமிக்க முடியவில்லை',
    failedToDelete: 'செயல்பாட்டை நீக்க முடியவில்லை',
    swipeToSwitch: '← சுயவிவரங்களை மாற்ற ஸ்வைப் செய்யவும் →',
    welcomeTitle: 'வரவேற்கிறோம்!',
    getStartedMessage: 'சுயவிவரத்தை உருவாக்குவதன் மூலம் தொடங்கவும்',
    noChildMessage: 'உங்கள் குழந்தையின் சுகாதார பயணத்தை கண்காணிக்க அவர்களின் சுயவிவரத்தை சேர்க்கவும்',
    addChild: 'குழந்தை சுயவிவரம் சேர்க்கவும்',
    pregnancyProfile: 'கர்ப்ப சுயவிவரம்',
    pregnancyDescription: 'உங்கள் கர்ப்ப பயணத்தை கண்காணிக்கவும்',
    childProfile: 'குழந்தை சுயவிவரம்',
    childDescription: 'உங்கள் குழந்தையின் சுகாதாரம் மற்றும் வளர்ச்சியை கண்காணிக்கவும்',
    createProfile: 'சுயவிவரம் உருவாக்கவும்',
    pregnancyProfileComingSoon: 'விரைவில் வருகிறது',
    pregnancyProfileMessage: 'கர்ப்ப சுயவிவர அம்சம் விரைவில் கிடைக்கும். உங்கள் குழந்தை பிறந்தவுடன் அவர்களின் சுயவிவரத்தை சேர்க்கலாம்.',
  },

  // Add Child Screen
  addChild: {
    title: 'குழந்தை சுயவிவரம் சேர்க்கவும்',
    firstNameRequired: 'முதல் பெயர் தேவை',
    lastNameRequired: 'கடைசி பெயர் தேவை',
    birthWeightRequired: 'சரியான பிறப்பு எடை தேவை',
    birthHeightRequired: 'சரியான பிறப்பு உயரம் தேவை',
    childAdded: 'குழந்தை சுயவிவரம் வெற்றிகரமாக உருவாக்கப்பட்டது',
    failedToCreate: 'குழந்தை சுயவிவரத்தை உருவாக்க முடியவில்லை',
    enterFirstName: 'முதல் பெயரை உள்ளிடவும்',
    enterLastName: 'கடைசி பெயரை உள்ளிடவும்',
    enterMotherName: 'தாயின் பெயரை உள்ளிடவும்',
    enterFatherName: 'தந்தையின் பெயரை உள்ளிடவும்',
    enterEmergencyContact: 'அவசர தொடர்பு எண்ணை உள்ளிடவும்',
  },

  // Profile Screen
  profile: {
    title: 'குழந்தை சுயவிவரம்',
    subtitle: 'தனிப்பட்ட தகவல்',
    personalInfo: 'தனிப்பட்ட தகவல்',
    birthInfo: 'பிறப்பு தகவல்',
    medicalInfo: 'மருத்துவ தகவல்',
    familyInfo: 'குடும்ப தகவல்',
    healthcareProviders: 'சுகாதார வழங்குநர்கள்',
    name: 'பெயர்',
    dateOfBirth: 'பிறந்த தேதி',
    gender: 'பாலினம்',
    male: 'ஆண்',
    female: 'பெண்',
    birthWeight: 'பிறப்பு எடை',
    birthHeight: 'பிறப்பு உயரம்',
    bloodType: 'இரத்த வகை',
    allergies: 'ஒவ்வாமைகள்',
    noneKnown: 'அறியப்படவில்லை',
    specialConditions: 'சிறப்பு நிலைமைகள்',
    none: 'இல்லை',
    motherName: 'தாயின் பெயர்',
    fatherName: 'தந்தையின் பெயர்',
    emergencyContact: 'அவசர தொடர்பு',
    assignedMidwife: 'நியமிக்கப்பட்ட மருத்துவச்சி',
    pediatrician: 'குழந்தை மருத்துவர்',
    editProfile: 'சுயவிவரத்தைத் திருத்து',
  },

  // Settings Screen
  settings: {
    title: 'அமைப்புகள்',
    subtitle: 'பயன்பாட்டு விருப்பத்தேர்வுகள்',
    language: 'மொழி',
    about: 'பற்றி',
    version: 'பதிப்பு',
    appName: 'பயன்பாட்டின் பெயர்',
  },

  // Vaccines Screen
  vaccines: {
    title: 'நோய்த்தடுப்பு',
    subtitle: 'தடுப்பூசி அட்டவணை',
    immunizationComplete: 'நோய்த்தடுப்பு நிறைவு',
    onTrack: 'சரியான பாதையில்',
    vaccineCard: 'தடுப்பூசி அட்டை',
    certificate: 'சான்றிதழ்',
    nextVaccination: 'அடுத்த தடுப்பூசி',
    scheduleAppointment: 'உங்கள் மருத்துவச்சியுடன் சந்திப்பு திட்டமிடுங்கள்',
    schedule: 'திட்டமிடு',
    status: {
      completed: 'நிறைவு',
      due: 'நிலுவை',
      upcoming: 'வரவிருக்கும்',
      overdue: 'தாமதம்',
      missed: 'தவறவிட்டது',
    },
    ageGroups: {
      birth: 'பிறப்பு',
      months2: '2 மாதங்கள்',
      months4: '4 மாதங்கள்',
      months6: '6 மாதங்கள்',
      months9: '9 மாதங்கள்',
      months12: '12 மாதங்கள்',
      months18: '18 மாதங்கள்',
    },
    given: 'கொடுக்கப்பட்டது:',
    dueDate: 'நிலுவை:',
  },

  // Growth Screen
  growth: {
    title: 'வளர்ச்சி கண்காணிப்பு',
    subtitle: 'வளர்ச்சி கண்காணிப்பான்',
    addMeasurement: 'அளவீடு சேர்',
    photoProgress: 'புகைப்பட முன்னேற்றம்',
    growthCharts: 'வளர்ச்சி விளக்கப்படங்கள்',
    weight: 'எடை',
    height: 'உயரம்',
    head: 'தலை',
    weightForAge: 'வயதுக்கான எடை',
    heightForAge: 'வயதுக்கான உயரம்',
    headCircumference: 'தலை சுற்றளவு',
    latest: 'சமீபத்திய:',
    atAge: '{{age}} மாதங்களில்',
    developmentMilestones: 'வளர்ச்சி மைல்கற்கள்',
    achieved: 'அடைந்தது:',
    expected: 'எதிர்பார்க்கப்படுகிறது:',
    growthStatus: 'வளர்ச்சி நிலை',
    growingWell: 'உங்கள் குழந்தை இயல்பான வரம்புகளில் நன்றாக வளர்கிறது',
    nextMeasurement: 'அடுத்த அளவீடு:',
    kg: 'கி.கி',
    cm: 'செ.மீ',
    percentile: '{{value}}வது',
  },

  // Feeding Screen
  feeding: {
    title: 'உணவு வழிகாட்டுதல்கள்',
    subtitle: 'CHDR ஊட்டச்சத்து வழிகாட்டி',
    selectAge: 'குழந்தையின் வயதைத் தேர்ந்தெடுக்கவும்',
    ageRanges: {
      months6_8: '6-8 மாதங்கள்',
      months9_11: '9-11 மாதங்கள்',
      months12_23: '12-23 மாதங்கள்',
    },
    texture: 'அமைப்பு',
    frequency: 'அதிர்வெண்',
    amountPerMeal: 'ஒரு உணவுக்கான அளவு',
    tipsTitle: 'பதிலளிக்கும் உணவளிப்புக்கான குறிப்புகள்',
    illnessTitle: 'நோயின் போது உணவளித்தல்',
    illnessDescription: 'நோயின் போது பொருத்தமற்ற உணவளிப்பு வளர்ச்சி தடுமாற்றத்திற்கு பொதுவான காரணமாகும். பொறுமையுடன் நோயின் போது உணவளிப்பதில் சிறப்பு கவனம் செலுத்துங்கள்.',
    feedingTip: 'உணவளிக்கும் நேரம் இனிமையான அனுபவமாக இருக்க வேண்டும். அது பராமரிப்பாளருக்கும் குழந்தைக்கும் இடையில் போராட்டமாக இருக்க வேண்டியதில்லை.',
    supplementTitle: 'திரிபோஷா சப்ளிமெண்ட்',
    eligibilityStatus: 'தகுதி நிலை',
    eligible: 'தகுதியானது',
    notEligible: 'தகுதியற்றது',
    receiving: 'பெறுகிறது',
    supplementDescription: 'திரிபோஷா என்பது ஆரோக்கியமான வளர்ச்சியை ஆதரிக்க சுகாதார அமைச்சகத்தால் வழங்கப்படும் ஊட்டச்சத்து நிரப்பியாகும். சரியான பயன்பாட்டு வழிகாட்டுதல்களுக்கு உங்கள் மருத்துவச்சியிடம் ஆலோசிக்கவும்.',
    viewDistributionSchedule: 'விநியோக அட்டவணையைக் காண்க',
  },

  // Schedule Screen
  schedule: {
    title: 'சந்திப்புகள்',
    subtitle: 'சுகாதார அட்டவணை',
    upcoming: 'வரவிருக்கும்',
    past: 'கடந்த',
    scheduleNew: 'புதிய சந்திப்பு திட்டமிடு',
    reschedule: 'மீள்திட்டமிடு',
    noUpcoming: 'வரவிருக்கும் சந்திப்புகள் இல்லை',
    noPast: 'கடந்த சந்திப்புகள் இல்லை',
    editAppointment: 'Edit Appointment',
    noChildren: 'No child profiles yet',
    noChildrenMessage: 'Add a child profile to schedule appointments.',
    quickActions: 'விரைவு செயல்கள்',
    findClinic: 'மருத்துவமனை கண்டுபிடி',
    emergency: 'அவசரம்',
    scheduled: 'திட்டமிடப்பட்டது',
    completed: 'நிறைவு',
    cancelled: 'ரத்து செய்யப்பட்டது',
    notes: 'குறிப்புகள்:',
    noContact: 'No contact number available for this clinic.',
    formChild: 'Child',
    formTitle: 'Title',
    formTitlePlaceholder: 'Appointment title',
    formType: 'Appointment Type',
    formDuration: 'Duration (minutes)',
    formLocation: 'Location',
    formLocationPlaceholder: 'Clinic or hospital',
    formAddress: 'Address',
    formAddressPlaceholder: 'Address (optional)',
    formProviderName: 'Provider Name',
    formProviderNamePlaceholder: 'Doctor or clinic',
    formProviderRole: 'Provider Role',
    formProviderRolePlaceholder: 'Specialist, nurse...',
    formProviderPhone: 'Provider Phone',
    formProviderPhonePlaceholder: 'Phone (optional)',
    formNotesPlaceholder: 'Additional notes (optional)',
    formRequired: 'Please fill in the required fields.',
    formSaveError: 'Unable to save appointment.',
  },

  // Roles
  roles: {
    pediatrician: 'குழந்தை மருத்துவர்',
    midwife: 'சமூக மருத்துவச்சி',
    ambulance: 'ஆம்புலன்ஸ்',
    communityHealthWorker: 'சமூக சுகாதார பணியாளர்',
  },

  // Time & Date
  time: {
    today: 'இன்று',
    yesterday: 'நேற்று',
    daysAgo: '{{days}} நாட்களுக்கு முன்',
    weeksAgo: '{{weeks}} வாரங்களுக்கு முன்',
    monthsAgo: '{{months}} மாதங்களுக்கு முன்',
    at: 'அன்று',
  },

  // Activities Screen
  activities: {
    title: 'அனைத்து செயல்பாடுகள்',
    noActivities: 'இன்னும் செயல்பாடுகள் இல்லை',
    noActivitiesMessage: 'நீங்கள் செயல்பாடுகளைச் சேர்க்கும்போது அவை இங்கே தோன்றும்',
    deleteError: 'செயல்பாட்டை நீக்க முடியவில்லை',
    deleteSuccess: 'செயல்பாடு வெற்றிகரமாக நீக்கப்பட்டது',
  },

  // Settings
  settings: {
    title: 'அமைப்புகள்',
    subtitle: 'பயன்பாட்டு விருப்பங்கள்',
    language: 'மொழி',
    notifications: 'அறிவிப்புகள்',
    darkMode: 'இருண்ட முறை',
    sync: 'தரவை ஒத்திசை',
    about: 'பற்றி',
    logout: 'வெளியேறு',
    version: 'பதிப்பு',
    appName: 'பயன்பாட்டு பெயர்',
    childProfiles: 'குழந்தை சுயவிவரங்கள்',
    childProfilesDescription: 'உங்கள் குழந்தைகளின் சுயவிவரங்களை நிர்வகிக்கவும். சுயவிவரங்களை மாற்ற முகப்புத் திரையில் இடது/வலது ஸ்வைப் செய்யவும்.',
    addAnotherChild: 'மற்றொரு குழந்தை சுயவிவரம் சேர்க்கவும்',
    addAnotherChildDescription: 'இரட்டையர்கள், உடன்பிறப்புகள் அல்லது மற்றொரு குழந்தைக்கு சுயவிவரம் சேர்க்கவும்',
    profileSwitched: 'குழந்தை சுயவிவரத்திற்கு மாற்றப்பட்டது',
    deleteChild: 'குழந்தை சுயவிவரத்தை நீக்கு',
    deleteChildConfirm: '{{name}} இன் சுயவிவரத்தை நீக்க விரும்புகிறீர்களா? இந்த செயலை செயல்தவிர்க்க முடியாது.',
    deleteChildFailed: 'குழந்தை சுயவிவரத்தை நீக்க முடியவில்லை',
    cannotDeleteLastChild: 'ஒரே குழந்தை சுயவிவரத்தை நீக்க முடியாது',
    pregnancyProfiles: 'கர்ப்ப சுயவிவரங்கள்',
  },

  // Errors
  errors: {
    networkError: 'பிணைய பிழை. உங்கள் இணைப்பை சரிபார்க்கவும்.',
    syncError: 'தரவை ஒத்திசைக்க முடியவில்லை. பின்னர் மீண்டும் முயற்சிக்கும்.',
    loadError: 'தரவை ஏற்ற முடியவில்லை.',
    saveError: 'மாற்றங்களைச் சேமிக்க முடியவில்லை.',
  },

  // Pregnancy Profile
  pregnancy: {
    createTitle: 'கர்ப்ப சுயவிவரத்தை உருவாக்கவும்',
    motherInfo: 'தாயாரின் தகவல்',
    pregnancyInfo: 'கர்ப்ப தகவல்',
    medicalInfo: 'மருத்துவ தகவல்',
    healthcareProviders: 'சுகாதார சேவை வழங்குநர்கள்',
    emergencyContact: 'அவசர தொடர்பு',
    additionalInfo: 'கூடுதல் தகவல்',
    motherFirstName: 'தாயாரின் முதல் பெயர்',
    motherLastName: 'தாயாரின் கடைசி பெயர்',
    motherDOB: 'தாயாரின் பிறந்த தேதி',
    bloodType: 'இரத்த வகை',
    expectedDeliveryDate: 'எதிர்பார்க்கப்படும் பிரசவ தேதி',
    lastMenstrualPeriod: 'கடைசி மாதவிடாய் காலம்',
    selectDate: 'தேதியைத் தேர்ந்தெடுக்கவும்',
    gravida: 'கர்ப்ப எண்',
    para: 'பிரசவ எண்',
    prePregnancyWeight: 'கர்ப்பத்திற்கு முந்தைய எடை',
    currentWeight: 'தற்போதைய எடை',
    height: 'உயரம்',
    enterWeight: 'எ.கா: 55',
    enterHeight: 'எ.கா: 165',
    medicalConditions: 'மருத்துவ நிலைமைகள்',
    medicalConditionsPlaceholder: 'எ.கா: நீரிழிவு, உயர் இரத்த அழுத்தம்',
    allergies: 'ஒவ்வாமைகள்',
    allergiesPlaceholder: 'எ.கா: பென்சிலின், வேர்க்கடலை',
    medications: 'தற்போதைய மருந்துகள்',
    medicationsPlaceholder: 'எ.கா: பிரசவ வைட்டமின்கள்',
    hospitalName: 'மருத்துவமனை/கிளினிக் பெயர்',
    enterHospitalName: 'மருத்துவமனை அல்லது கிளினிக் பெயரை உள்ளிடவும்',
    obgynName: 'மகப்பேறு மருத்துவர் பெயர்',
    enterObgynName: 'மருத்துவர் பெயரை உள்ளிடவும்',
    obgynContact: 'மகப்பேறு மருத்துவர் தொடர்பு',
    enterObgynContact: 'தொடர்பு எண்ணை உள்ளிடவும்',
    midwifeName: 'மருத்துவச்சி பெயர்',
    enterMidwifeName: 'மருத்துவச்சி பெயரை உள்ளிடவும்',
    midwifeContact: 'மருத்துவச்சி தொடர்பு',
    enterMidwifeContact: 'தொடர்பு எண்ணை உள்ளிடவும்',
    emergencyContactName: 'தொடர்பு பெயர்',
    enterEmergencyContactName: 'பெயரை உள்ளிடவும்',
    emergencyContactPhone: 'தொடர்பு தொலைபேசி',
    enterEmergencyContactPhone: 'தொலைபேசி எண்ணை உள்ளிடவும்',
    emergencyContactRelation: 'உறவு',
    enterEmergencyContactRelation: 'எ.கா: கணவர், பெற்றோர், உடன்பிறப்பு',
    partnerName: 'துணையின் பெயர்',
    createProfile: 'கர்ப்ப சுயவிவரத்தை உருவாக்கவும்',
    enterMotherFirstName: 'தாயாரின் முதல் பெயரை உள்ளிடவும்',
    enterMotherLastName: 'தாயாரின் கடைசி பெயரை உள்ளிடவும்',
    enterPartnerName: 'துணையின் பெயரை உள்ளிடவும்',
    enterEmergencyContact: 'அவசர தொடர்பு எண்ணை உள்ளிடவும்',
    motherFirstNameRequired: 'தாயாரின் முதல் பெயர் தேவை',
    motherLastNameRequired: 'தாயாரின் கடைசி பெயர் தேவை',
    profileCreated: 'கர்ப்ப சுயவிவரம் வெற்றிகரமாக உருவாக்கப்பட்டது',
    failedToCreate: 'கர்ப்ப சுயவிவரத்தை உருவாக்க முடியவில்லை. மீண்டும் முயற்சிக்கவும்.',
    networkError: 'பிணைய பிழை. உங்கள் இணைய இணைப்பைச் சரிபார்த்து மீண்டும் முயற்சிக்கவும்.',
    authError: 'அங்கீகாரப் பிழை. வெளியேறி மீண்டும் உள்நுழையவும்.',
    serverError: 'சேவையக பிழை. பின்னர் மீண்டும் முயற்சிக்கவும்.',
    // Dashboard
    dashboard: 'கர்ப்ப கண்காணிப்பாளர்',
    hello: 'வணக்கம்',
    momToBe: 'எதிர்கால தாய்',
    youAreInWeek: 'நீங்கள் வாரத்தில் உள்ளீர்கள்',
    week: 'வாரம்',
    weeks: 'வாரங்கள்',
    weeksAnd: 'வாரங்கள் &',
    days: 'நாட்கள்',
    daysRemaining: 'பிரசவத்திற்கு மீதமுள்ள நாட்கள்',
    firstTrimester: '1வது மூன்று மாதம்',
    secondTrimester: '2வது மூன்று மாதம்',
    thirdTrimester: '3வது மூன்று மாதம்',
    trimester: 'மூன்று மாதம்',
    expectedDate: 'எதிர்பார்க்கப்படும்',
    babyDevelopment: 'குழந்தை வளர்ச்சி',
    babySize: 'அளவு',
    motherHealth: 'தாயின் சுகாதாரம்',
    hospital: 'மருத்துவமனை',
    doctor: 'மருத்துவர்',
    noProfile: 'கர்ப்ப சுயவிவரம் இல்லை',
    createProfileMessage: 'உங்கள் பயணத்தைக் கண்காணிக்க கர்ப்ப சுயவிவரத்தை உருவாக்கவும்.',
    trackYourVisits: 'உங்கள் வருகைகளைக் கண்காணிக்கவும்',
    // Checkups Screen
    checkups: 'பிரசவத்திற்கு முந்தைய பரிசோதனைகள்',
    noCheckups: 'கர்ப்ப சுயவிவரம் இல்லை',
    createProfileForCheckups: 'உங்கள் பிரசவத்திற்கு முந்தைய பரிசோதனைகளைக் கண்காணிக்க கர்ப்ப சுயவிவரத்தை உருவாக்கவும்.',
    switchToCurrent: 'தற்போதைய கர்ப்ப சுயவிவரத்திற்கு மாறவும்',
    viewingCompletedTapSwitch: 'நீங்கள் நிறைவடைந்த கர்ப்பத்தைப் பார்க்கிறீர்கள். உங்கள் தற்போதைய கர்ப்பத்திற்கு மாற தட்டவும்.',
    yourProgress: 'உங்கள் முன்னேற்றம்',
    checkupsCompleted: 'பரிசோதனைகள் முடிந்தன',
    upcoming: 'வரவிருக்கும்',
    checkupSchedule: 'பரிசோதனை அட்டவணை',
    now: 'இப்போது',
    // Health Screen
    readOnly: 'படிக்க மட்டும்',
    cannotEditCompleted: 'நிறைவடைந்த கர்ப்ப சுயவிவரங்களைத் திருத்த முடியாது.',
    viewingCompleted: 'நிறைவடைந்த கர்ப்பத்தைப் பார்க்கிறது',
    tapToSwitchCurrent: 'தற்போதைய கர்ப்பத்திற்கு மாற தட்டவும்',
    viewingCompletedReadOnly: 'நிறைவடைந்த கர்ப்பத்தைப் பார்க்கிறது (படிக்க மட்டும்)',
    symptomsNoteReadOnly: 'வரலாற்று அறிகுறிகள் தரவைப் பார்க்கிறது.',
    // Convert to Child Modal
    congratulations: 'வாழ்த்துக்கள்!',
    dueDatePassed: 'உங்கள் பிரசவ தேதி வந்துவிட்டது! உங்கள் குழந்தையை விரைவில் வரவேற்பீர்கள் என்று நம்புகிறோம்.',
    dueDatePassedPersonal: 'அன்பான {{name}}, உங்கள் பிரசவ தேதி வந்துவிட்டது! உங்கள் குழந்தையை விரைவில் வரவேற்பீர்கள் என்று நம்புகிறோம்.',
    readyToCreateChild: 'உங்கள் குழந்தையின் வளர்ச்சியைக் கண்காணிக்கத் தயாரா?',
    dueDate: 'பிரசவ தேதி',
    createChildProfile: 'குழந்தை சுயவிவரத்தை உருவாக்கு',
    maybeLater: 'பின்னர் ஒருவேளை',
    canCreateLater: 'அமைப்புகளிலிருந்து எப்போது வேண்டுமானாலும் குழந்தை சுயவிவரத்தை உருவாக்கலாம்.',
    // Health Screen
    health: 'உடல்நலம் மற்றும் நல்வாழ்வு',
    weightTracking: 'எடை கண்காணிப்பு',
    prePregnancy: 'கர்ப்பத்திற்கு முன்',
    current: 'தற்போதைய',
    gained: 'பெற்றது',
    currentWeight: 'தற்போதைய எடை (kg)',
    symptomsToday: 'இன்றைய அறிகுறிகள்',
    history: 'வரலாறு',
    symptomsNote: 'உங்கள் அறிகுறிகளைப் பதிவு செய்ய தட்டவும். அவை தானாகவே சேமிக்கப்படும்.',
    wellnessTips: 'நல்வாழ்வு குறிப்புகள்',
    medicalInfo: 'மருத்துவ தகவல்',
    conditions: 'நோய் நிலைகள்',
    noConditions: 'நோய் நிலைகள் பதிவு செய்யப்படவில்லை',
    noAllergies: 'ஒவ்வாமைகள் பதிவு செய்யப்படவில்லை',
    symptomsHistory: 'அறிகுறிகள் வரலாறு',
    noSymptomsHistory: 'அறிகுறிகள் வரலாறு இல்லை',
    editConditions: 'நோய் நிலைகளைத் திருத்து',
    editAllergies: 'ஒவ்வாமைகளைத் திருத்து',
    enterCondition: 'நோய் நிலையை உள்ளிடவும்',
    enterAllergy: 'ஒவ்வாமையை உள்ளிடவும்',
    noConditionsYet: 'நோய் நிலைகள் இன்னும் சேர்க்கப்படவில்லை',
    noAllergiesYet: 'ஒவ்வாமைகள் இன்னும் சேர்க்கப்படவில்லை',
    noHealthData: 'கர்ப்ப சுயவிவரம் இல்லை',
    createProfileForHealth: 'உங்கள் உடல்நலத்தைக் கண்காணிக்க கர்ப்ப சுயவிவரத்தை உருவாக்கவும்.',
    failedToSaveSymptoms: 'அறிகுறிகளைச் சேமிக்க தோல்வி',
    failedToLoadHistory: 'அறிகுறிகள் வரலாற்றை ஏற்ற தோல்வி',
    itemExists: 'இந்த உருப்படி ஏற்கனவே உள்ளது',
    medicalInfoUpdated: 'மருத்துவ தகவல் புதுப்பிக்கப்பட்டது',
    failedToUpdateMedicalInfo: 'மருத்துவ தகவலை புதுப்பிக்க தோல்வி',
    invalidWeight: 'செல்லுபடியாகும் எடையை உள்ளிடவும்',
    weightUpdated: 'எடை வெற்றிகரமாக புதுப்பிக்கப்பட்டது',
    failedToUpdateWeight: 'எடையை புதுப்பிக்க தோல்வி',
  },
};
